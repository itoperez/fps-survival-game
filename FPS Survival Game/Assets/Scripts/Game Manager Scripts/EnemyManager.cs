...
public class EnemyManager : MonoBehaviour
{
    ...
    private GameObject[] all_Enemy_Prefabs;
    ...
    public void ClearIsCurrentEnemiesFlag(bool is_Cannibal)
    {
        if (all_Enemy_Prefabs == null)
        {
            all_Enemy_Prefabs = GameObject.FindGameObjectsWithTag(Tags.ENEMY_TAG);

            for (int i = 0; i < all_Enemy_Prefabs.Length; i++)
            {
                if (is_Cannibal && all_Enemy_Prefabs[i].name == Tags.ENEMY_CANNIBAL_TAG)
                {
                    all_Enemy_Prefabs[i].GetComponent<EnemyStats>().is_Current_Cannibal = false;
                } else
                {
                    all_Enemy_Prefabs[i].GetComponent<EnemyStats>().is_Current_Boar = false;
                }
            }
        }

        all_Enemy_Prefabs = null;
